...
# Kill all Next.js dev servers whose working directory is under $REPO_ROOT,
# plus anything bound to the given target port. Removes stale .next/dev/lock.
#
# Why: Next.js 16 (Turbopack) refuses to start a second dev server in the same
# project directory even on a different port. Detecting "next-server" PIDs and
# only killing those matching the target port leaves cross-port stragglers that
# block the new server. Scope kills to this project to avoid hitting neighbor
# projects running next-server elsewhere on the machine.
#
# Args: $1 = target frontend port (defaults to CHAIN_FRONTEND_PORT or 3000)
# Safe to call multiple times; idempotent.
kill_project_next_servers() {
  local target_port="${1:-${CHAIN_FRONTEND_PORT:-3000}}"
  local project_root
  project_root="$(cd "$REPO_ROOT" && pwd -P)"

  local pids pid cwd
  pids=$(pgrep -f "next-server|next dev|npm exec next" 2>/dev/null || true)
  local killed_any=""
  for pid in $pids; do
    cwd=$(readlink "/proc/$pid/cwd" 2>/dev/null || true)
    # Only kill if the process's cwd is inside this project's directory tree.
  ...
  if [[ -n "$killed_any" ]]; then
    sleep 2
    # Second pass with KILL for anything that didn't honour TERM.
    for pid in $pids; do
      cwd=$(readlink "/proc/$pid/cwd" 2>/dev/null || true)
      if [[ -n "$cwd" && "$cwd" == "$project_root"* ]]; then
  ...
  # Also free the target port (covers anything we missed above).
  fuser -k "${target_port}/tcp" 2>/dev/null || true
  # Remove stale Next.js dev lock if the owning PID is gone.
  local lock_file="$project_root/apps/frontend/.next/dev/lock"
  if [[ -f "$lock_file" ]]; then
    local lock_pid
    lock_pid=$(grep -oE '"pid":[0-9]+' "$lock_file" 2>/dev/null | head -1 | cut -d: -f2 || true)
