...
15. `runs/goal-session-<sid>/iter-<N>/side-effects.json` — the deterministic side-effect ledger (HARD-3), named on your prompt's "Side-effect ledger" line whenever side-effect context applies: the spec's `Side-effect policy`, which journeys are MUTATING (declared by the owner, or OBSERVED by the replay lane — an observation always beats a `none` declaration), NONE or Unknown, and the declaration digest. Read it with methodology A.8.
...
**Side effects are not failures (HARD-3).** Do not fail the product journey merely because one of its own declared numbered steps mutates state (a run it launches, a row it appends). If that required mutation conflicts with the iteration spec — a TC asserting "row count unchanged", an OUT OF SCOPE "no new run" — classify it as a **spec/journey contradiction**, not a product regression: the deterministic preflight should normally have blocked such a spec before execution, so its presence is itself worth reporting. Score such a TC on the invariant that matters (no PRE-EXISTING row edited or deleted), and name the contradiction in `## Summary` and in `assumptions.md` (step 5b). Never ignore an actual contradiction, and never excuse a mutation that is NOT one of the journey's own numbered steps — that stays a finding. A journey the owner declared `none` that the replay OBSERVED mutating (your ledger line says `DECLARATION CONFLICT`) is the opposite case: report it as a finding — a product regression or a wrong declaration — never as the journey's own step.
...
- Run the 6-item pre-finalize self-check (methodology section E) before writing eval.md. If any item fails, fix the evaluation — do not ship it with a caveat.
