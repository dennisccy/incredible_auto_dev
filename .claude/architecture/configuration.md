...
| `CHAIN_REQUIRE_FULL_DEPTH` | (unset) | **OPERATOR-SET ONLY.** Truthy (`true`/`TRUE`/`1`/`yes`/`on`) makes full depth a HARD requirement for every iteration of the run: it outranks every cost rung of the arbiter, and if full still cannot be dispatched the engine pauses `AWAITING_FULL_DEPTH` before dispatch instead of running lean. Per-iteration equivalent: a `Depth enforcement: required` line in the iteration spec. The decomposer is forbidden to emit that line (anti-pattern 25) — a human writes it. Default off: with neither present the arbiter behaves exactly as before |
| `CHAIN_MAINTENANCE_ISOLATION` | (unset) | **OPERATOR-SET ONLY.** Truthy (`true`/`TRUE`/`1`/`yes`/`on`/`required`) declares the run a maintenance isolation: full reviewer/QA/auditor/coherence/evaluator depth REQUIRED, application-service boot, browser QA, the deterministic replay lane and the demo showcase FORBIDDEN. Enforced fail-closed at six chokepoints: `detect_frontend_in_plan` subordinates the `CHAIN_GOAL_TARGET_JOURNEYS` browser override, and `_boot_shared_services`, `ensure_services_running`, `browser-qa-phase.sh`, `replay_lane_partition_and_verify` and `demo-phase.sh` call `maintenance_isolation_refuse`, which appends to `iter-<N>/maintenance-isolation-refusals` and emits a `maintenance_isolation_refused` event. Per-iteration equivalent: a `Maintenance isolation: required` line in the spec (decomposer forbidden — anti-pattern 25) |
