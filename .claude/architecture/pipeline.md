...
- **Signal / quota propagation.** A signal exit (130/137/143) inside either branch is forwarded by `lib/parallel.sh::parallel_run` to the other branch as SIGTERM, then propagated up so `run-phase.sh`'s outer signal guard aborts the run cleanly; the next resume re-runs the fanout. Reserved lifecycle halts rank next, by meaning rather than numeric magnitude — browser evidence gate unavailable (79) > canonical spec field unavailable (78) > dispatch unavailable (70) — so a simultaneous generic failure (e.g. 124) or quota on the other branch can never mask one into a warn-and-continue that records `post_dev_parallel_complete` (whose resume would skip the whole UI/browser chain). Quota exhaustion (exit 75) then propagates immediately so the outer `_run_step` quota loop can sleep and retry; any other codes aggregate as the numeric max.
