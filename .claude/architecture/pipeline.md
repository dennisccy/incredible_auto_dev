...
- **Signal / quota propagation.** A signal exit (130/137/143) inside either branch is forwarded by `lib/parallel.sh::parallel_run` to the other branch as SIGTERM, then propagated up so `run-phase.sh`'s outer signal guard aborts the run cleanly; the next resume re-runs the fanout. Reserved lifecycle halts rank next, by meaning rather than numeric magnitude — browser evidence gate unavailable (79) > canonical spec field unavailable (78) > dispatch unavailable (70) — so neither a simultaneous generic failure (e.g. 124) — which would become a warn-and-continue that records `post_dev_parallel_complete` (whose resume would skip the whole UI/browser chain) — nor quota on the other branch — which would report a generic quota stop instead of the specific halt, its marker and its remedy — can mask one. Quota exhaustion (exit 75) then propagates: `run-phase.sh` exits 75 with its checkpoint unchanged (typically `review_passed`), and under goal mode `run-goal.sh` stops resumably before evaluation (`ABORTED`, halt `QUOTA_EXHAUSTED`); a later resume re-runs the fanout. Automatic quota waiting stays with the layers below (`claude_with_quota_retry`, `_run_step`); any other codes aggregate as the numeric max.
