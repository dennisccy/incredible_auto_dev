...
**Quota exhaustion is NOT a halt.** The wrapped `claude_with_quota_retry` library transparently sleeps until the quota resets, then resumes the same agent invocation. Telemetry records the quota pause for observability. When a FULL executor (`run-phase.sh`) exits `QUOTA_EXHAUSTED_EXIT_CODE` (75) because an agent wrapper gave up on quota, the iteration did not finish: the engine waits the way `run-phase.sh`'s `_run_step` does (the shared sentinel's reset epoch, else `CHAIN_CLAUDE_FALLBACK_SLEEP_SECONDS`) and re-dispatches the same iteration, which resumes from `runs/<iter>/status.json`; the coherence auditor, the goal-evaluator, the `current_iter` advance and the per-iteration push never run over a FULL executor that exited 75. With `CHAIN_DISABLE_AUTO_WAIT=true` (or a zero wait) the engine stops resumably as `ABORTED` instead (halt reason `QUOTA_EXHAUSTED`). Not covered: quota a run-phase step absorbs itself (a leaf SKIPPED stub, or a bounded step retry that moves on) never reaches the engine; `_run_step`'s own dev/review/QA/audit retry loops keep waiting and retrying inside `run-phase.sh` (they ignore `CHAIN_DISABLE_AUTO_WAIT`); and the lean executor has no such handling — a lean exit 75 still proceeds to evaluation.
