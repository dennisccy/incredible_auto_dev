...
Generates:
  ...
  <dir>/.claude/settings.json  (deny-only, one per policy `claude_launch_dirs` entry)
...
# ── Launch-directory settings ─────────────────────────────────────────────────
# Claude Code reads the SHARED .claude/settings.json only from the directory a
# session is started in (https://code.claude.com/docs/en/settings: "to use a file
# committed at the repository root, start Claude Code there"). A session started
# in a product subdirectory — apps/backend, say — therefore loads NONE of the
# rendered settings: not this policy's deny list, not the hooks, nothing.
#
# A product that supports such a launch lists the directory in
# policy/permissions.yaml `claude_launch_dirs` (paths relative to the project
# root). Each existing one gets a generated <dir>/.claude/settings.json carrying
# the SAME deny list, so a session started there enforces the same deny rules.
# Deny ONLY: a subdirectory session gains protections, never permissions — no
# allow rules, no hooks, no additional directories. The files are generated and
# covered by `sync-cli-assets.py --check`, so a re-render cannot drop them.

def _launch_dirs() -> list[Path]:
    raw = T.load_permissions().get("claude_launch_dirs") or []
    if not isinstance(raw, list):
        raise SystemExit("policy/permissions.yaml: claude_launch_dirs must be a list of directories")
    dirs: list[Path] = []
    for entry in raw:
        rel = Path(str(entry).strip())
        if not str(entry).strip() or rel.is_absolute() or ".." in rel.parts or rel == Path("."):
            raise SystemExit(
                f"policy/permissions.yaml: claude_launch_dirs entry {entry!r} must be a "
                "subdirectory of the project root (relative, no '..')")
        dirs.append(rel)
    return dirs


def render_launch_dir_settings_json(rel_dir: Path) -> str:
    source = os.path.relpath(T.NEUTRAL_POLICY / "permissions.yaml", T.PROJECT_ROOT / rel_dir)
    settings = {
        "_comment": (
            "GENERATED by sync-cli-assets.py --cli claude from " + source + " (deny, "
            "claude_launch_dirs) — do not edit; edit the policy and re-render. Claude Code "
            "reads .claude/settings.json only from the directory a session starts in, so "
            "this file gives sessions started in " + rel_dir.as_posix() + " the project's "
            "deny rules. Deny only: no allow rules, hooks or directories."
        ),
        "permissions": {"deny": list(T.load_permissions().get("deny", []))},
    }
    return json.dumps(settings, indent=2, ensure_ascii=False) + "\n"


def sync_launch_dir_settings(*, dry_run: bool = False) -> int:
    changed = 0
    for rel in _launch_dirs():
        base = T.PROJECT_ROOT / rel
        if not base.is_dir():
            # Nothing can start a session in a directory that does not exist (e.g. a
            # checkout of the framework subtree alone), so there is nothing to protect.
            continue
        target = base / ".claude" / "settings.json"
        rendered = render_launch_dir_settings_json(rel)
...
def sync_all(*, dry_run: bool = False) -> dict[str, int]:
    return {
        ...
        "launch_dirs": sync_launch_dir_settings(dry_run=dry_run),
    }
