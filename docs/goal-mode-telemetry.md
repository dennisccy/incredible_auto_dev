...
| Field | Type | Description |
|---|---|---|
| `depth` | string | `lean`, `full`, or `evidence` (HARD-1: the lean executor reports `evidence` on the evidence micro-path) |
...
| Field | Type | Description |
|---|---|---|
| `reason` | string | Includes `BUDGET_EXHAUSTED`, `STALLED`, `REGRESSION_HALT`, `ABORT_MALFORMED`, `DECOMPOSER_FAILED`, `QUOTA_EXHAUSTED` (a FULL executor propagated quota exhaustion to the engine after the quota handling below it gave up — `detected_at_step:"executor"`, `rc`, `iter_name`; session status `ABORTED`, resumable; the engine neither waits nor re-dispatches), `GATE_BLOCKED_POST_DECOMPOSE`, `machine_reset`, and the resumable pauses `AWAITING_BLUEPRINT_APPROVAL`, `AWAITING_INTENT_REVIEW`, `AWAITING_PUMP`, `AWAITING_GITHUB_AUTH`, `AWAITING_DISK`, `AWAITING_HOST_GUARD`, `AWAITING_FULL_DEPTH`. `ABORTED` is a session *status* only — the SIGINT trap writes the summary, not a halt event. Not a closed enum: `grep -n 'record_telemetry_event "halt"' scripts/automation/run-goal.sh` is the ground truth |
...
|---|---|---|
| `step_skipped` | `goal-iter-lean.sh`, `run-goal.sh`, `run-phase.sh` | `{step, iter_name|phase, reason}` — a step was skipped instead of dispatched. Reasons: `checkpoint` (resume reused a completed step), `evidence-mode` (HARD-1: the evidence micro-path deliberately did not dispatch `developer`/`reviewer` — never reported as `checkpoint`), `zero-change` (SPEED-14), `iter-budget-trim` (SPEED-15 rungs 3a/3b: `test-plan`/`ux-regression`, payload key `phase`), `ui-combined` (SPEED-24: `ui-test-design` folded into the ui-impact dispatch, payload key `phase`) |
...
| `depth_evidence_override` | `run-goal.sh` | `{from:"lean", to:"evidence", target_journeys, work_guard}` — the SPEED-9 evidence backstop demoted a lean dispatch whose Target journeys are all recorded passing AND (HARD-1) whose spec plans no Backend/Frontend work under `## IN SCOPE` (`lib/iter_spec.py has-implementation-work`); `work_guard` records the `CHAIN_EVIDENCE_WORK_GUARD` value in force |
| `depth_evidence_refused` | `run-goal.sh` | `{site, from, to, target_journeys?, implementation_work}` — HARD-1: an evidence dispatch was refused because the spec plans implementation work (`implementation_work` is the probe's JSON: backend/frontend/loose bullet counts). `site:"backstop"` = the lean→evidence demotion was declined (depth stays lean); `site:"spec-declared"` = a `Depth: evidence` spec was dispatched lean; `site:"executor-belt"` = goal-iter-lean.sh refused with exit 76 and the engine re-dispatched lean — this site must be zero in healthy sessions (it means the engine-side guards were bypassed) |
| `depth_escalate_override` | `run-goal.sh` | `{from:"lean"\|"evidence", to:"full", prior_verdict:"ESCALATE"}` — HARD-1: a lean/evidence spec written after an ESCALATE was promoted to full before dispatch (`CHAIN_ESCALATE_FORCES_FULL`, default on; iteration 0 exempt) |
| `evidence_mode_refused` | `goal-iter-lean.sh` | `{iter_name, reason:"implementation-work-in-spec", implementation_work}` — HARD-1 belt: the executor refused an evidence-only dispatch BEFORE writing any handoff/review/browser artifact; marker `runs/goal-session-<sid>/iter-<N>/evidence-mode-refused` carries `reason=`, `spec=`, `work=` |
| `spec_lint` | `run-goal.sh` | `{iter_name, attempt, rc, errors, warnings, mode, side_effect_policy, side_effect_rules, prohibitions, mutating, restrictive}` — HARD-2: the deterministic spec lint ran over this iteration's spec after the goal-decomposer wrote it and BEFORE any dispatch. `rc` 0 = clean or warnings only, 1 = lint ERRORs, 2 = linter crash / unreadable spec. `attempt` is 1 or 2 (2 = the one automatic re-plan). `mode` is the `CHAIN_SPEC_LINT` value in force. HARD-3 fields: `side_effect_policy` = the spec's `Side-effect policy` value as the canonical parser read it — lower-cased, bold stripped (`""` when absent or written in a form the parser cannot read, which is itself `E02`), `side_effect_rules` = the side-effect rule ids that fired (`E02` only when it names the policy line, `E06 E13 E14 E15 E16 E17 W02 W09 W10 W11`, sorted; `E17` is HARD-3 B3's retention rule and needs no ledger), `prohibitions` = how many explicit no-mutation prohibitions the spec carries, `mutating` = the checked journeys (targets ∪ required ∪ make-up) whose status was `mutating` and `restrictive` = whether the policy was read as restrictive (anything but a plain `allowed`, optionally followed by a dash note — in any label shape, and also when such a line sits inside what the parser reads as a code fence or HTML comment) — all `null` when the preflight did not run, i.e. `CHAIN_SIDE_EFFECT_PREFLIGHT=false`. **Tripwire (framework bug, stop):** an `attempt:2` event with `side_effect_policy:"allowed"`, `prohibitions > 0` and a non-empty `mutating` list whose `side_effect_rules` lacks `E16` |
| `spec_obligation_pinned` | `run-goal.sh` (spec lint) | `{iter_name, attempt, journeys, scope:"iteration"}` — HARD-3 B3: this iteration's FIRST spec was rejected under E13/E16 and the finding named these **Required-still-passing** journeys. They stay a binding verification obligation for the rest of THIS iteration: the re-planned spec must still name each of them in `Target journeys:` or `Required-still-passing journeys:` (either field — both are verified — or the engine's make-up set), or `E17` rejects it. Recorded in the iteration-scoped `iter-<N>/spec-obligations.json`, so a resume re-reads it and another iteration never sees it. At most one per iteration; zero is the normal case |
| `spec_obligation_dropped` | `run-goal.sh` (spec lint) | `{iter_name, attempt, dropped, pinned, rule:"E17"}` — HARD-3 B3: the spec being linted names a pinned journey in NEITHER journey field, i.e. the re-plan tried to make the contradiction disappear by deleting the id instead of resolving it. In `block` mode this is an ordinary lint ERROR, so attempt 2 halts `GATE_BLOCKED` with `detected_at_step:"spec-lint"` and nothing is dispatched; the event is recorded whatever the mode does next. **Tripwire (planner contract, look):** any occurrence means the decomposer answered a side-effect contradiction by dropping a required journey |
| `spec_obligation_unreadable` | `run-goal.sh` (spec lint) | `{iter_name, attempt, path}` — HARD-3 B3: `iter-<N>/spec-obligations.json` EXISTS but carries no readable journey list, so the iteration's retention obligation cannot be checked. The engine halts `GATE_BLOCKED` with `detected_at_step:"spec-obligations"` BEFORE any dispatch rather than treating an unreadable obligation as no obligation (the same rule as E15: a safety layer must not vanish because its own input broke). The record is written atomically, so this should never fire; deleting the file is the deliberate way to retire an obligation |
| `spec_obligation_unrecorded` | `run-goal.sh` (spec lint) | `{iter_name, attempt, journeys, error, rc}` — HARD-3 B3: the rejected first spec named these Required-still-passing journeys, but the obligation could NOT be persisted to `iter-<N>/spec-obligations.json` (an unwritable path, a stale `.tmp` of the wrong type, a full filesystem — `error` carries the exception, `rc` the recorder's exit code). The engine halts `GATE_BLOCKED` with `detected_at_step:"spec-obligations"` **before the re-plan runs**, because re-planning with no recorded retention is exactly the dodge E17 exists to stop. Must be zero in healthy sessions |
| `spec_replan` | `run-goal.sh` | `{iter_name, errors}` — HARD-2: lint ERRORs bought the session its ONE automatic re-plan; the decomposer was re-dispatched with the errors quoted back. Tripwire: more than 1 in 5 iterations means the decomposer contract wording needs work, not the gate |
| `halt` (`GATE_BLOCKED_SPEC_FIELD_UNAVAILABLE`) | `run-goal.sh` | `{reason, detected_at_step:"executor-spec-field", rc:78, iter_name}` — HARD-2: the executor exited `SPEC_FIELD_UNAVAILABLE_EXIT_CODE` because a canonical machine field could not be read from the spec's `## Goal Mode Metadata` section at runtime. Distinct from `spec_lint_crash` (the lint may have passed earlier) and from `AWAITING_PUMP` (this is not transport). The coherence auditor and the evaluator do NOT run, `current_iter` is not advanced, and `iter-<N>/spec-field-unavailable` records the reason |
| `spec_lint_crash` | `run-goal.sh` | `{iter_name, rc, mode, stderr_tail}` — HARD-2: the linter itself failed or the spec could not be read (HARD-3: also recorded before an E15 halt that ends a lint which did not finish — a crash exit, or a traceback's exit 1 with no finished JSON). In `block` mode this halts `GATE_BLOCKED` with `detected_at_step:"spec-lint-crash"` and is NEVER re-planned; in `warn` mode the engine continues and says so. Must be zero in healthy sessions |
| `halt` (`GATE_BLOCKED_SIDE_EFFECT_LEDGER`) | `run-goal.sh` | `{reason, detected_at_step:"side-effect-ledger", lint_mode, lint_rc}` — HARD-3 E15: the spec declares `Side-effect policy: none` but `iter-<N>/side-effects.json` could not be built or read, is incomplete (corrupt sidecar or per-run record, unreadable exception file) or is stale (not this run's preflight build). A restrictive policy is never trusted without its evidence source, so the engine halts BEFORE any dispatch and does NOT re-plan (the planner cannot repair the input) — in `CHAIN_SPEC_LINT=warn` too (`lint_mode` records which), and whatever the lint's exit code (`lint_rc`): when the lint did not finish its side-effect pass, the engine reads the policy (`iter_spec.py policy-intent`) and the ledger (`iter_spec.py ledger-ok`) itself and halts when the policy reads as `none` or cannot be read and the ledger is not usable. A policy line that reads as anything but a plain `allowed` (a dash note may follow), in any label shape, counts as restrictive. When the halt ends a lint that did not finish (a crash exit, or a traceback's exit 1 without a finished JSON), `spec_lint_crash` is recorded first. Only the announced switches `CHAIN_SPEC_LINT=off` and `CHAIN_SIDE_EFFECT_PREFLIGHT=false` skip the check. `--resume` rebuilds the ledger and re-lints the same spec |
...
### Side-effect events (HARD-3)

Written by `lib/replay-lane.sh` (observations, from the replay lane's per-run record
`iter-<N>/replay-side-effects.json`) and by `run-goal.sh` (ledger build and preflight).

| Event | Written by | Payload |
|---|---|---|
| `side_effect_observed` | `lib/replay-lane.sh` | `{iter_name, journey, mutating_count, auth_count, readonly_count, sample, complete, golden}` — one per journey per replay run (a REL-5 retry or a redone forked lane is a second run), zero counts included. `sample` = up to 3 `METHOD /path` mutating requests (paths only, never a query string or body). `complete` = the replay passed end-to-end with the observer attached; `golden` = the first 12 hex of the replayed golden script's identity. Only a complete clean replay of the SAME golden clears an earlier mutation |
| `side_effect_exception_applied` | `lib/replay-lane.sh` | `{iter_name, journey, kind, method, path}` — a request was NOT counted as a mutation: `kind:"read-only"` = it matched an owner-authored line of `project-extensions/side-effects/read-only-endpoints.txt`; `kind:"auth"` = it is session plumbing on the auth/session exclusion list: POST or DELETE on an excluded endpoint itself, one sign-in step below it (`/auth/login`, `/token/refresh`, `/auth/callback`) or a NextAuth provider form (`/auth/callback/<provider>`) — never anything else under an auth word (`/auth/register`, `/auth/users`, `/session/all` are mutations). Every suppression is reported; there is no silent exception |
| `side_effect_clear_refused` | `lib/replay-lane.sh` | `{iter_name, journey, golden, mutating_golden, mutating_iter, sample}` — a complete clean replay could NOT clear a mutation recorded earlier under a different (or unidentified) golden script, so the journey stays `mutating` (for example a re-derived golden that no longer clicks Run) |
| `side_effect_sidecar_update_failed` | `lib/replay-lane.sh` | `{iter_name, journeys, message}` — the replay could not update the sidecar (lock timeout after `CHAIN_SIDE_EFFECT_LOCK_TIMEOUT`, corrupt or wrongly-shaped file). Nothing is lost: the per-run record keeps the observations and the next preflight merges them (`side_effect_observations_repaired`) |
| `side_effect_declaration_changed` | `run-goal.sh` (preflight ledger build) | `{journey, from, to, iter, iter_name, note_changed, declaration_digest}` — a journey's `- Side effects:` declaration in `docs/goal.md` changed since the engine last recorded it (`from`/`to` ∈ `none`/`mutating`/`unknown`; a note-only edit has `from == to` and `note_changed:true`). The same event with `journey:null, source:"read-only-endpoints"` and 12-hex `from`/`to` digests records an edit to the exception file, and with `source:"auth-ignore-paths"` (`from`/`to` = the path lists) a change to the `CHAIN_SIDE_EFFECT_IGNORE_PATHS` override (`rejected` lists entries that were refused because they name `/` or an API root, or are not one plain path) — both can suppress observations, so neither changes silently. Declarations are journey-hash-neutral, so this event — not goal-edit drift — is their provenance trail. Nothing is emitted on the first record of a session |
| `side_effect_declaration_conflict` | `run-goal.sh` (preflight ledger build) | `{journey, declared:"none", observed, observed_iter, sticky, iter, iter_name}` — a journey the owner declared `none` was observed mutating. Emitted once per distinct observation; the evaluator is told on every iteration to report it as a finding (regression or wrong declaration) |
| `side_effect_observations_repaired` | `run-goal.sh` (preflight ledger build) | `{iter, iter_name, runs, records}` — the preflight merged per-run records the sidecar had not merged (a failed update, a sidecar moved aside). Should be rare |
| `side_effect_unknown` | `run-goal.sh` (spec lint) | `{iter_name, attempt, journeys, count, policy, strict}` — journeys the iteration executes (targets ∪ required ∪ make-up) whose side-effect status is unknown (no valid declaration, no observation). Should trend to zero as declarations are added |
| `side_effect_ledger_unavailable` | `run-goal.sh` (ledger build) | `{iter_name, step, rc, reason}` — `step` = `preflight` or `pre-evaluator`; `rc` 3 = the ledger was written but is incomplete (`reason` names what could not be established), any other `rc` = no ledger was written. Under `Side-effect policy: none` the preflight then fails closed (E15, `GATE_BLOCKED_SIDE_EFFECT_LEDGER`); under `allowed` or no policy it is W11 and dispatch continues. Must be zero in healthy sessions |

**Reading them.** `side_effect_observed` per journey should be stable across iterations — a
`mutating_count` flip from 0 to N (or back) is either a product change or a golden that
changed what it exercises. E13 and E16 counts come from `spec_lint.side_effect_rules`; the
TenSteps iteration-9 contradiction is an E16 (an explicit "no new run" prohibition against a
mutating J-04), plus an E13 when the spec also declared `none`. W10 counts (prohibition +
unknown journeys) and `side_effect_unknown` are the signal that declarations are missing.
`side_effect_clear_refused`, `side_effect_sidecar_update_failed` and
`side_effect_observations_repaired` explain why a journey is still `mutating`, or why the
observations had to be rebuilt; `side_effect_declaration_conflict` is always worth an owner
look (a product that started writing, or a wrong `none`).
