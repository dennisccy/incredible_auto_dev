...
| `spec_lint` | `run-goal.sh` | `{iter_name, attempt, rc, errors, warnings, mode}` — HARD-2: the deterministic spec lint ran over this iteration's spec after the goal-decomposer wrote it and BEFORE any dispatch. `rc` 0 = clean or warnings only, 1 = lint ERRORs, 2 = linter crash / unreadable spec. `attempt` is 1 or 2 (2 = the one automatic re-plan). `mode` is the `CHAIN_SPEC_LINT` value in force |
