...
| `spec_lint` | `run-goal.sh` | `{iter_name, attempt, rc, errors, warnings, mode, side_effect_policy, side_effect_rules, prohibitions}` — HARD-2: the deterministic spec lint ran over this iteration's spec after the goal-decomposer wrote it and BEFORE any dispatch. `rc` 0 = clean or warnings only, 1 = lint ERRORs, 2 = linter crash / unreadable spec. `attempt` is 1 or 2 (2 = the one automatic re-plan). `mode` is the `CHAIN_SPEC_LINT` value in force. HARD-3 fields: `side_effect_policy` = the spec's declared `Side-effect policy` as written (`""` when absent), `side_effect_rules` = the side-effect rule ids that fired (`E06 E13 E14 E15 E16 W02 W09 W10 W11`, sorted), `prohibitions` = how many explicit no-mutation prohibitions the spec carries (`null` when the preflight did not run, i.e. `CHAIN_SIDE_EFFECT_PREFLIGHT=false`). **Tripwire (framework bug, stop):** an `attempt:2` event whose `side_effect_policy` flipped to `allowed` while `prohibitions > 0` and a mutating journey was checked, yet `side_effect_rules` lacks `E16` |
...
| `halt` (`GATE_BLOCKED_SIDE_EFFECT_LEDGER`) | `run-goal.sh` | `{reason, detected_at_step:"side-effect-ledger"}` — HARD-3 E15: the spec declares `Side-effect policy: none` but `iter-<N>/side-effects.json` could not be built or read, or is incomplete (corrupt sidecar, unreadable exception file). A restrictive policy is never trusted without its evidence source, so the engine halts in `block` mode BEFORE any dispatch and does NOT re-plan (the planner cannot repair the input). `--resume` rebuilds the ledger and re-lints the same spec. In `CHAIN_SPEC_LINT=warn` the engine continues and says `continuing UNVERIFIED` |
...
|---|---|---|
| `side_effect_observed` | `lib/replay-lane.sh` | `{iter_name, journey, mutating_count, auth_count, readonly_count, sample, complete}` — one per journey the deterministic replay actually replayed, zero counts included. `sample` = up to 3 `METHOD /path` mutating requests (paths only, never a query string or body). `complete` = the replay passed end-to-end with the observer attached (only such an observation can clear an earlier mutation) |
| `side_effect_exception_applied` | `lib/replay-lane.sh` | `{iter_name, journey, method, path}` — a request matched an owner-authored line of `project-extensions/side-effects/read-only-endpoints.txt` and was NOT counted as a mutation. Every suppression is reported; there is no silent exception |
| `side_effect_declaration_changed` | `run-goal.sh` (preflight ledger build) | `{journey, from, to, iter, iter_name, note_changed, declaration_digest}` — a journey's `- Side effects:` declaration in `docs/goal.md` changed since the engine last recorded it (`from`/`to` ∈ `none`/`mutating`/`unknown`; a note-only edit has `from == to` and `note_changed:true`). The same event with `journey:null, source:"read-only-endpoints"` and 12-hex `from`/`to` digests records an edit to the exception file, and with `source:"auth-ignore-paths"` (`from`/`to` = the path lists) a change to the `CHAIN_SIDE_EFFECT_IGNORE_PATHS` override — both can suppress observations, so neither changes silently. Declarations are journey-hash-neutral, so this event — not goal-edit drift — is their provenance trail. Nothing is emitted on the first record of a session |
