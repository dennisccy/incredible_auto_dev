...
|---|---|---|
| `side_effect_observed` | `lib/replay-lane.sh` | `{iter_name, journey, mutating_count, auth_count, readonly_count, sample, complete, golden}` — one per journey the deterministic replay actually replayed, zero counts included. `sample` = up to 3 `METHOD /path` mutating requests (paths only, never a query string or body). `complete` = the replay passed end-to-end with the observer attached; `golden` = the first 12 hex of the replayed golden script's identity. Only a complete clean replay of the SAME golden clears an earlier mutation |
