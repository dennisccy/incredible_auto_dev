...
| `side_effect_exception_applied` | `lib/replay-lane.sh` | `{iter_name, journey, kind, method, path}` — a request was NOT counted as a mutation: `kind:"read-only"` = it matched an owner-authored line of `project-extensions/side-effects/read-only-endpoints.txt`; `kind:"auth"` = it is session plumbing on the auth/session exclusion list (POST/DELETE only, never a request naming a resource id). Every suppression is reported; there is no silent exception |
